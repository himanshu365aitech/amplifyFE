...
const page = () => {
  ...
  // Use effect to check if the participant is in the waiting room and redirect them
useEffect(() => {
  const participantInWaitingRoom = waitingRoom?.some(
    (participant) => participant?.name === fullName && participant?.role === userRole
  );

  if (participantInWaitingRoom) {
    // Redirect to participant waiting room page
    router.push(
      `/participant-waiting-room/${params.id}?fullName=${encodeURIComponent(fullName)}&role=Participant`
    );
  }
}, [waitingRoom, fullName, userRole, params.id, router]);
  ...
  const moveUserToWaitingRoom = async(name, role, meetingId) => {
    console.log('move user to waiting room', name, role, meetingId)
    try {
      response = await axios.put(
        `https://amplifymeetingbe.onrender.com/api/live-meeting/move-participant-to-waiting-room`,
        {
          name: name,
          role: role,
          meetingId: meetingId,
      ...
      console.log('response for waiting room move', response)

      if(response.data.message === "Participant moved to the waiting room"){
         notify("success", "Success", `${userName.name} has been removed`);
      }
          ...
          <>
            <div className="h-full">
              <LeftSidebar
                ...
                moveUserToWaitingRoom={moveUserToWaitingRoom}
              />
            </div>
